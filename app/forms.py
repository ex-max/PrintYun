...
class Print(FlaskForm):
    ...
    print_place = SelectField('打印点', choices=[('home', '数智维新工作室')])

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        # 下单表单每次实例化时从数据库拉取最新的启用中打印点，
        # 未建表/为空时回退到硬编码默认值，保证老环境不炸
        try:
            from app.models import PrintPlace
            rows = (PrintPlace.query
                    .filter_by(Is_Active=True)
                    .order_by(PrintPlace.Sort.asc(), PrintPlace.Id.asc())
                    .all())
            if rows:
                self.print_place.choices = [(p.Key, p.Name) for p in rows]
        except Exception:
            pass
    print_time = DateField('预约时间')
    print_copies = IntegerField('打印份数', default=1)
    print_demand = SelectField('打印需求', choices=[('0' , '需要排版'), ('1' , '直接打印')], default='1')
